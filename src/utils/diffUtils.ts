 ...
 */
export function formatDiffForConsole(diff: FileDiff, contextLines: number = 3): string {
  if (!diff.hasChanges) {
 ...
 */
function groupDiffLines(diffLines: DiffLine[], contextLines: number): DiffLine[][] {
 ...
 */
export function promptUserApproval(message: string = 'Apply these changes?'): Promise<boolean> {
 ...
 */
export function promptUserChoice(message: string, choices: string[]): Promise<string> {
  return new Promise((resolve) => {
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout
 ...
 */
export function mergeSections(oldSections: ContentSection[], newSections: ContentSection[]): string {
