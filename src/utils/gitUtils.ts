 ...
 */
export function getChangesSinceDate(basePath: string, sinceDate: Date): GitChanges {
  const repoInfo = getRepositoryInfo(basePath);
  
  if (!repoInfo.hasGit) {
    console.warn('⚠️  Git not available - using file system timestamps for change detection');
 ...
 */
function getChangesFromFileSystem(basePath: string, sinceDate: Date): GitChanges {
  const changes: GitChanges = {
    newFiles: [],
    modifiedFiles: [],
    deletedFiles: []
...
function isRelevantFile(filePath: string): boolean {
  const relevantExtensions = ['.ts', '.js', '.tsx', '.jsx', '.md', '.json', '.yaml', '.yml'];
