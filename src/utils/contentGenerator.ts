  ...
  try {
    ...
    for (const item of items) {
      if (item.isFile()) {
        ...
        if (item.name.endsWith('.ts') || item.name.endsWith('.js') || item.name.endsWith('.tsx') || item.name.endsWith('.jsx')) {
 ...
 */
export async function generateKnowledgeContent(dirPath: string): Promise<string | null> {
  try {
