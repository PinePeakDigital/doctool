...
describe("Knowledge Manager", () => {
  ...
  describe("scanDirectory", () => {
    it("should scan a directory and return correct information", () => {
      ...
      expect(result.subdirectories).toEqual(
  ...
  describe("getDirectoriesForKnowledgeFiles", () => {
    ...
    it("should find all non-excluded directories recursively", () => {
      ...
      expect(dirNames).toEqual(
        expect.arrayContaining(["src", "tests", "src/utils", "src/agents"]),
      );
  ...
  describe("createKnowledgeFile", () => {
    it("should create a knowledge file when none exists", () => {
      ...
      const content = fs.readFileSync(
        path.join(tempDir, "knowledge.md"),
        "utf8",
      );
    ...
    it("should not create a file when one already exists", () => {
      ...
      expect(fs.readFileSync(path.join(tempDir, "KNOWLEDGE.md"), "utf8")).toBe(
        "existing content",
      );
    });
  });
  ...
  describe("initializeKnowledgeFiles", () => {
    ...
    it("should initialize knowledge files for all directories", () => {
      ...
      expect(fs.existsSync(path.join(tempDir, "knowledge.md"))).toBe(true);
      expect(
        fs.existsSync(path.join(tempDir, "src", "utils", "knowledge.md")),
      ).toBe(true);
      expect(fs.existsSync(path.join(tempDir, "tests", "knowledge.md"))).toBe(
        true,
      );

      // Check that existing file was not overwritten
      expect(
        fs.readFileSync(path.join(tempDir, "src", "KNOWLEDGE.md"), "utf8"),
      ).toBe("existing");

      // Check console output
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("Scanning directories in:"),
      );
      expect(consoleSpy).toHaveBeenCalledWith(
    ...
    it("should handle directories where all knowledge files already exist", () => {
      ...
      fs.writeFileSync(
        path.join(tempDir, "src", "utils", "knowledge.md"),
        "utils",
      );
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("Knowledge files created: 0"),
      );
