...
Check the config/ folder for settings.
      ...
      expect(
        references.some(
          (ref) => ref.path === "src/utils/" && ref.type === "directory",
        ),
      ).toBe(true);
      expect(
        references.some(
          (ref) => ref.path === "config/" && ref.type === "directory",
        ),
      ).toBe(true);
    });
...
\`\`\`
      ...
      const claims = validator.extractDirectoryStructureClaims(
        content,
        "test.md",
      );
...
\`\`\`
  ...
  describe("validateFileReference", () => {
    beforeEach(() => {
      ...
      fs.writeFileSync(
        path.join(tempDir, "existing-dir", "nested-file.txt"),
        "content",
      );
    });
  ...
  describe("validateDirectoryStructure", () => {
    ...
    it("should detect missing files in directory structure", () => {
      ...
      const missingFileIssues = issues.filter(
        (i) => i.type === "missing_file" && i.message.includes("missing.ts"),
      );
...
This references a [missing file](nonexistent.txt).
      ...
      const existingFileIssues = issues.filter(
        (issue) =>
  ...
  describe("path type detection", () => {
    it("should correctly guess file vs directory types", () => {
      ...
      testCases.forEach(({ path, expected }) => {
        const ref = validator.extractFileReferences(
          `\`${path}\``,
          "test.md",
        )[0];
      ...
      const noExtensionRefs = validator.extractFileReferences(
        "`no-extension`",
        "test.md",
      );
