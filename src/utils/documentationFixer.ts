...
export function applyDocumentationFixes(
  ...
  const issuesToFix = analysis.issues.filter(
    (issue) => severityLevels[issue.severity] >= threshold,
  );

  // Apply fixes in order of severity (high -> medium -> low)
  const sortedIssues = issuesToFix.sort(
    (a, b) => severityLevels[b.severity] - severityLevels[a.severity],
  );

  for (const issue of sortedIssues) {
    const result = applyIssuseFix(issue, modifiedContent, {
      dryRun,
      autoApprove,
    });
    results.push(result);
