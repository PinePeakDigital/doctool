...
export class FileSystemValidator {
  ...
  public validateDocumentationFile(docFilePath: string): ValidationIssue[] {
    try {
      ...
      for (const fileRef of fileReferences) {
        const validationIssues = this.validateFileReference(
          fileRef,
          docFilePath,
        );
      ...
      const directoryClaims = this.extractDirectoryStructureClaims(
        content,
        docFilePath,
      );
  ...
  public extractFileReferences(
    ...
    lines.forEach((line, lineIndex) => {
      ...
      while ((match = directoryRegex.exec(line)) !== null) {
        const possiblePath = match[1];
        if (
          this.looksLikeDirectoryPath(possiblePath) &&
          possiblePath.length > 2
        ) {
          references.push({
            path: possiblePath,
            type: "directory",
      ...
      const commonFileRegex =
        /(?:^|\s)((?:package\.json|\.env|\.gitignore|Dockerfile|Makefile|README\.md|LICENSE|tsconfig\.json|\.npmrc|\.editorconfig)(?:\s|$|[,.])?)/g;
   ...
   */
  public extractDirectoryStructureClaims(
    content: string,
    sourceFile: string,
  ): DirectoryStructureClaim[] {
      ...
      if (trimmedLine.startsWith("```")) {
        if (!inDirectoryTree) {
          ...
          if (currentClaim.length > 0) {
            const directoryPath =
              this.inferDirectoryFromTree(currentClaim) || ".";
            claims.push({
  ...
  public validateDirectoryStructure(
    claim: DirectoryStructureClaim,
  ): ValidationIssue[] {
    const issues: ValidationIssue[] = [];

    try {
      const actualStructure = this.getActualDirectoryStructure(
        claim.directory_path,
      );
      claim.actual_structure = actualStructure;

      const claimedFiles = this.extractFilesFromTreeStructure(
        claim.claimed_structure,
      );
